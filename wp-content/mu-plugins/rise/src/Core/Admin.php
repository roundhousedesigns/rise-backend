<?php
	...
	class Admin {
		...
		public function enqueue_styles() {
			...
			if ( $current_screen && 'tools_page_rise-csv-import' === $current_screen->id ) {
				\wp_enqueue_style(
		...
		/**
		 * Handle login redirects for all user roles.
		 *
		 * Redirects administrators to the admin area and crew-members to the frontend.
		 *
		...
		public function handle_login_redirects( $redirect_to, $request, $user ) {
			...
			if ( !is_wp_error( $user ) && is_object( $user ) && isset( $user->ID ) ) {
				...
				elseif ( in_array( 'crew-member', $user->roles ) ) {
